// npm run test:module1 - copy-pasta to test run the fun
...
const dateDiff = (date1, date2) => {
    date1 = date1.getTime(); //maybe needs to be: let newDate1 = new Date(date1) - we will see
    date2 = date2.getTime();

    let difTime = Math.abs( date1 - date2 ); //Math.abs makes all integers positive
    let difDays = Math.ceil( difTime / (1000 * 60 * 60 * 24) ) //Round up after dividing the difference by milliseconds in a day = days
    // let difDays = Math.ceil(Math.abs(date1.getDate() - date2.getDate()))
    return difDays;
};
...
//     }
// }
//This second example is more in-line with what the directions were asking for
...
const loadProducts = (map, prodId) => {
    ...
    try {
        ...
        // prodKeys.push(Object.keys(new Product()));why does this not work
